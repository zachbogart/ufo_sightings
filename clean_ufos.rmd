...
The dataset was scraped from the [National UFO Reporting Center](http://www.nuforc.org/). Initially, the data was quite disorganized. Here is a look at the raw data:
...
Before reading in the data, we considered `unknown` to be considered as an `NA` value.
```{r}
...
Finally, since the `Shape` column had so many different kinds included, we separated them into 
```{r}
