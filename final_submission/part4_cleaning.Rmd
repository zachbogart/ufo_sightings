...
```{r pull-in-data}
# read in raw data
ufo <- as_tibble(read_csv("data/ufo_data.csv", na=c('unknown', '', 'Unknown')))
#Bad Eggs
...
Finally, since the `Shape` column had so many different kinds included, we consolidated them. We had different opinions on what bins would work well for analysis, so we included multiple versions.
```{r shape-binning}
...
sapply(ufo, class)
write.csv(ufo, file = "data/ufo_clean.csv")
```

```{r final-results}
ufo_clean <- as_tibble(read_csv("data/ufo_clean.csv"))
summary(ufo_clean)
```
