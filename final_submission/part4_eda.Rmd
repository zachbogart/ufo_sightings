...
ggplot(ufodurclip, aes(x = Duration)) + geom_histogram(, color="black", fill="green") +
  ggtitle("Duration Histogram, Limited to 1 Hour")
```

As expected, the majority of UFO sighting happen between 6pm and midnight. Late enough that it's dark, but early enough that people haven't gone to sleep yet.
```{r freq_over_time, echo = FALSE}
ggplot(ufo, aes(as.POSIXct(Time))) +
  geom_histogram(bins=48, color="black", fill="green") +
...
More sightings seeem to happen during months with warmer temperatures.People tend to spend more time outside during warmer months.. coincidence? 
```{r freq_by_month, echo=FALSE}
ufoAmerica <- ufo %>% filter(is.na(In_USA) == FALSE && In_USA == TRUE)
ggplot(ufo, aes(month)) +
  geom_bar(bins=12, color="black", fill="green") + 
...
Average word count of each comment centers around 100 words, but is it correlated other features?
...
ggplot(ufoShortDescription, aes(DescriptionLength)) +
  geom_histogram(bins=70, color="black", fill="green") +
  ggtitle("UFO Sighting Frequency By Comment Word Count") +
  labs(y = "Frequency", x = "Word Count") + scale_fill_viridis(name = "Frequency")

```

The word count doesn't seem to be affected by Time or Shape, but the Shape sure does change over time. Durig the day we don't see any examples of `light`, while at night we see a mix of everything. We can also see clear rounding happening at every hour.
...
Again, word count doesn't seem to be correlated with other features. This graph serves to show that when the website form went up in 1998 there was a huge spike in data.
